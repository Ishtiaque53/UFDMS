<?php
    ...
    $quary = "SELECT personalnum, rank, username from `user` where appt = 'MT' and todate is null ";
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        echo '<script>alert("Old MT NCO not found");</script>';
        header("refresh:.1; url=qm_home.php");
    }

    $quary = "SELECT personalnum, rank, username, password from `user_req` where appt = 'MT' and status = 'requested'";
    $result = mysqli_query($con, $quary) or die(mysqli_error($con));
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        echo '<script>alert("No MT NCO has requested");</script>';
        header("refresh:.1; url=qm_home.php");
    }
     ...
     if(isset($_POST["approve"])) {
         ...
         $quary = "UPDATE `user` SET todate = '".$fromDate->format('Y-m-d H:i:s')."' WHERE appt = 'MT' and todate is null";
         $result1 = mysqli_query($con, $quary) or die(mysqli_error($con));

         $quary = "INSERT INTO user (personalnum, rank, username, password, fromdate, appt) VALUES ('".$snNo."', '".$rank."', '".$name."', '".$password."', '".$fromDate->format('Y-m-d H:i:s')."', 'MT')";
         $result2 = mysqli_query($con, $quary) or die(mysqli_error($con));

         $quary = "UPDATE `user_req` SET status = 'approved' WHERE appt = 'MT' and status = 'requested'";
         $result3 = mysqli_query($con, $quary) or die(mysqli_error($con));

         if($result1 && $result2 && $result3) {
             $quary = "commit";
             mysqli_query($con, $quary) or die(mysqli_error($con));
             echo '<script>alert("MT NCO Change Successful");</script>';
             header("refresh:.1; url=qm_home.php");
         }
             ...
             $quary = "rollback";
             mysqli_query($con, $quary) or die(mysqli_error($con));
             echo '<script>alert("MT NCO Change was not successful");</script>';
         }
    }
    ...
    if(isset($_POST["not-approved"])) {
        ...
        if($result4) {
            $quary = "commit";
            mysqli_query($con, $quary) or die(mysqli_error($con));
            echo '<script>alert("MT NCO was not approved");</script>';
            header("refresh:.1; url=qm_home.php");
        }
...
<head>
    ...
    <title>MT NCO Handing</title>
    <link rel="stylesheet" href="css/qm_mt_handing.css">
</head>
<body>
<?php include 'inc.style.php'; ?>
    <section class="hero">
        <div class="container1">
            <div class="left-col">
                <p class="subhead">MT NCO</p>
                <form action="qm_mt_handing.php" method="post">
                    <div class="old">
                        <h3>OLD MT NCO</h3>
