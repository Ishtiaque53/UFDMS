<?php
    ...
    if(isset($_POST["submit"])) {
        ...
        $quary = "Select status from `user_req` WHERE appt = 'POL' AND status = 'requested'";
        ...
        $quary = "INSERT INTO user_req (personalnum, rank, username, password, appt, status) VALUES ('".$soinikno."', '".$rank."', '".$name."', '".$password."', 'POL', 'requested')";
        $result1 = mysqli_query($con, $quary) or die(mysqli_error($con));

        $quary = "INSERT INTO qmcomments (comment_subject, comment_text, comment_status, comment_link) VALUES ('NCO Change Request', 'POL NCO', '0', 'qm_pol_handing.php')";
        $result2 = mysqli_query($con, $quary) or die(mysqli_error($con));

        if($result1 && $result2) {
            $quary = "commit";
            mysqli_query($con, $quary) or die(mysqli_error($con));
            echo '<script>alert("POL NCO Change Requested");</script>';
            header("refresh:.1; url=pol_home.php");
        }

        else {
            $quary = "rollback";
            mysqli_query($con, $quary) or die(mysqli_error($con));
            echo '<script>alert("POL NCO Change could not be requested");</script>';
        }
...
<head>
    ...
    <title>POL NCO Change</title>
    <link rel="stylesheet" href="css/pol_change.css">
</head>
<body>
<?php include 'inc.style.php'; ?>
    <section class="hero">
        <div class="container1">
            <div class="left-col">
                <p class="subhead">POL NCO Change</p>
                <form action="pol_change.php" method="post">
